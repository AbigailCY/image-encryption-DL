...
if __name__ == '__main__':
    ...
    # t = time.time()
    with PdfPages('./graph/'+'alexnet_cifar'+'_hist.pdf') as pdf: 
        for epoch in range(1, 2):
            ...
            print('----------------start test-----------------')
            fc3, fc2, fc1, conv5, conv4, conv3, conv2, conv1 = test(model, test_loader)
            print('----------------end test-----------------')
            ...
            plt.subplot(181).set_title('Eopch: '+ str(epoch)+' Layer FC3')
            ...
            plt.xticks(fontsize=6)
            plt.yticks(fontsize=6)
            plt.subplot(183).set_title('Eopch: '+ str(epoch)+' Layer FC1')

            plt.subplot(184)
            weights = np.ones_like(conv5)/float(len(conv5))
            plt.hist(conv5, bins = 100, weights=weights)
            plt.xticks(fontsize=6)
            plt.yticks(fontsize=6)
            plt.subplot(184).set_title('Eopch: '+ str(epoch)+' Layer conv5')

            plt.subplot(185)
            weights = np.ones_like(conv4)/float(len(conv4))
            plt.hist(conv4, bins = 100, weights=weights)
            plt.xticks(fontsize=6)
            plt.yticks(fontsize=6)
            plt.subplot(185).set_title('Eopch: '+ str(epoch)+' Layer conv4')

            plt.subplot(186)
            weights = np.ones_like(conv3)/float(len(conv3))
            plt.hist(conv3, bins = 100, weights=weights)
            plt.xticks(fontsize=6)
            plt.yticks(fontsize=6)
            plt.subplot(186).set_title('Eopch: '+ str(epoch)+' Layer conv3')
            ...
            plt.xticks(fontsize=6)
            plt.yticks(fontsize=6)
            plt.subplot(187).set_title('Eopch: '+ str(epoch)+' Layer conv2')
            ...
            plt.xticks(fontsize=6)
            plt.yticks(fontsize=6)
            plt.subplot(188).set_title('Eopch: '+ str(epoch)+' Layer conv1')
