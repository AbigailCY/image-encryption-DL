...
class Module_hessian(nn.Module):
    ...
    def recursive_eig_pre_hessian(self, W, h, eig_H2):
        ...
        if W == 'identity':
            eig_W2 = B
            eig_H1 = eig_W2 * eig_H2
        else:
            m, n = W.shape
            bs = h.shape[0]
            eig_H1 = torch.zeros(bs, n).cuda()

            # WB2 = B.view(bs, 1, n) * W
            # eig_W2 = (WB2*W).sum(2)
            eig_W2 = (B.view(bs, 1, n) * W.pow(2)).sum(2)

            eig_H1_cal = eig_W2.view(bs, -1, 1) * eig_H2.view(bs, 1, -1)

            if m > n:
                eig_H1_cal = eig_H1_cal.view(bs, -1).topk(n)[0]
                eig_H1 = eig_H1_cal[:, -n:]
            elif n > m:
                eig_H1_cal = eig_H1_cal.view(bs, -1).topk(m)[0]
                eig_H1[:, -m:] = eig_H1_cal

        return eig_H1

    # def gmean(self, x, dim):
    #     log_x = torch.log(x)
    ...
    def eig_hessian_cal(self, x, eig_H2):
        bs, d = x.shape

        eig_x2 = x.view(bs, 1, -1).matmul(x.view(bs, -1, 1)).view(-1, 1)
        eig_hessian = eig_x2 * eig_H2
        eig_hessian = self.gmean(eig_hessian)
    ...
    def register_hessian(self, name, H):
        self._hessians[name] = H.view(self.H_shape[0])
...
class Relu_FC(Module_hessian):
    ...
    def compute_hessian(self, h_hessian):
        ...
        h = self.x
        x = self.x_relu
        hessian = self.eig_hessian_cal(x, h_hessian)
        h_hessian = self.recursive_eig_pre_hessian(W, h, h_hessian)
...
class Relu_hessian(Module_hessian):
    ...
    def compute_hessian(self, h_hessian):
        ...
        h_hessian = self.recursive_eig_pre_hessian('identity', h, h_hessian)
...
class Max_Relu_FC(Module_hessian):
    ...
    def forward(self, x):
        ...
        self.x_relu = self.x_relu.view(x.shape[0], -1)
        self.h = self.fc(self.x_relu)
    ...
    def compute_hessian(self, h_hessian):
        ...
        x = self.x_relu
        hessian = self.eig_hessian_cal(x, h_hessian)
        h_hessian = self.recursive_eig_pre_hessian(W, h.view(h.shape[0], -1), h_hessian)
        self.register_hessian('conv.weight.hessian', hessian)

        h_hessian = h_hessian.view(h.shape)
        h_hessian = F.max_unpool2d(h_hessian, self.inds_pool, kernel_size=self.kernel_size_p,
                                   stride=self.stride_p, padding=self.padding_p, output_size=self.x.shape[2:4])
...
class Max_Relu(Module_hessian):
    ...
    def compute_hessian(self, h_hessian):
        ...
        h_hessian = self.recursive_eig_pre_hessian('identity', h.view(h.shape[0], -1), h_hessian)
