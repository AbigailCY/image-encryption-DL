...
from models.hessian_cal2 import *
...
class ResNet34_hessian(Module_hessian):
    def __init__(self, num_classes=1000, pretrain=False):
        super(ResNet34_hessian, self).__init__()
        block = BasicBlock
        layers = [2, 2, 2, 2]
...
class ResNet50_hessian(Module_hessian):
    def __init__(self, num_classes=1000, pretrain=False):
        super(ResNet50_hessian, self).__init__()
        block = Bottleneck
        layers = [3, 4, 6, 3]
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = Relu_FC(512 * block.expansion, num_classes)
        ...
        if pretrain:
            state_dict = torch.load('./models/pretrain_models/resnet50-19c8e357.pth', map_location='cpu')
            state_dict.pop('fc.weight')
            state_dict.pop('fc.bias')
