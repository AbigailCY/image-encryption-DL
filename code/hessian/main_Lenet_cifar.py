...
if __name__ == '__main__':
    ...
    toSave3 = np.zeros((20,10,84))
    toSave2 = np.zeros((20,84,120))
    toSave1 = np.zeros((20,120,400))
    toSave4 = np.zeros(20)
    for epoch in range(1, 21):
        ...
        toSave3[epoch-1,:,:] = fc3
        toSave4[epoch-1] = acc
    np.save("./hessian_eigen/LC_fc1.npy", toSave1)
    np.save("./hessian_eigen/LC_fc2.npy", toSave2)
    np.save("./hessian_eigen/LC_fc3.npy", toSave3)
    np.save("./hessian_eigen/LC_acc.npy", toSave4)




    # with PdfPages('./cifar_lenet.pdf') as pdf: 
    #     d = pdf.infodict()
    #     d['Title'] = 'Hessian ESD: Lenet FC1 on Cifar'
    #     for epoch in range(1, 21):
    #         print('----------------start train-----------------')
    #         fc1, fc2, fc3 = train(model, train_loader, optimizer, epoch)
    #         print('----------------end train-----------------')
    ...
    #         plt.subplot(133).set_title('Eopch: '+ str(epoch)+" Layer FC3",overhead=0.1)

    #         plt.tight_layout()
    #         pdf.savefig()
    #         plt.close()

    torch.save(model.state_dict(), "./checkpoint/lenet_cifar.pkl")
